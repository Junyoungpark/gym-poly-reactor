...
class PolyReactor(gym.Env):
    ...
    def step(self, action):
        ...
        # m_a_dot = (m_dot_f * w_AF) - (k_r1 * (m_a - ((m_a * m_AWT) / (m_w + m_a + m_p)))) - (p_1 * k_r2 * (m_a / m_ges) * m_AWT)
        m_a_new = m_a + self.tau * m_a_dot

        # 3rd ODE
        m_p_dot = k_r1 * m_a_r + p_1 * k_r2 * m_AWT * m_a / m_ges
        # m_p_dot = (k_r1 * (m_a-((m_a*m_AWT)/(m_w+m_a+m_p)))) + (p_1 * k_r2 * (m_a/m_ges) * m_AWT)
        ...
        # T_S_dot = 1. / (c_pS * m_S) * ((k_K * A_tank * (T_R - T_S)) - (k_K * A_tank * (T_S - T_M)))
        ...
        # 7th ODE
        _7th_ode_mul = (fm_AWT * c_pR * (T_R - T_EK) - alfa * (T_EK - T_AWT) + k_r2 * m_a * m_AWT * delH_R / m_ges)
        T_EK_dot = 1.0 / (c_pR * m_AWT) * _7th_ode_mul

        # T_EK_dot = 1. / (c_pR * m_AWT) * ((fm_AWT * c_pR * (T_R - T_EK)) - (alfa * (T_EK - T_AWT)) + (
        #         p_1 * k_r2 * (m_a / m_ges) * m_AWT * delH_R))

        T_EK_new = T_EK + self.tau * T_EK_dot
        # T_EK_new = np.clip(T_EK_new, a_min=T_EK_MIN, a_max=T_EK_MAX)
        ...
        # T_AWT_dot = 1. / (c_pW * m_AWT_KW) * ((fm_AWT_KW * c_pW * (t_awt_in - T_AWT)) - (alfa * (T_AWT - T_EK)))
        T_AWT_new = T_AWT + self.tau * T_AWT_dot
        # T_AWT_new = np.clip(T_AWT_new, a_min=T_AWT_MIN, a_max=T_AWT_MAX)

        # SAFETY VARIABLES
        # 9th ODE
        T_adiab_dot = delH_R / (m_ges * c_pR) * m_a_dot - (m_w_dot + m_a_dot + m_p_dot) * (
                (m_a * delH_R) / (m_ges * m_ges * c_pR)) + T_R_dot
        # T_adiab_dot = delH_R / (m_ges * c_pR) * m_a_dot - (m_w_dot + m_a_dot + m_p_dot) * (
        #             m_a * delH_R / (m_ges * m_ges * c_pR)) + T_R_dot
        ...
        self.state = np.squeeze(np.array(state_new))
    ...
    def reset(self, random_init=False):
        x0 = [m_W_INIT, m_A_INIT, m_P_INIT,
              ...
              T_AWT_INIT, T_adiab_INIT, m_acc_F_INIT]
        x0 = np.array(x0)
        if random_init:
